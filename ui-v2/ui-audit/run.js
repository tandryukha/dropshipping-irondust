...
(async () => {
  ...
  for (const s of sizes) {
    ...
    await page.screenshot({ path: path.join(outDir, `pdp-${s.label}-${s.width}.png`), fullPage: true });
  }

  const { AxeBuilder } = require('@axe-core/playwright');
  const axeResults = await new AxeBuilder({ page }).analyze();
  fs.writeFileSync(path.join(outDir, 'axe-index.json'), JSON.stringify(axeResults, null, 2));
